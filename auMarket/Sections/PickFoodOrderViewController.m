...
@interface PickFoodOrderViewController ()

@end

@implementation PickFoodOrderViewController
...
-(void)setUpTableView{
    float table_height=HEIGHT_SCREEN-HEIGHT_STATUS_AND_NAVIGATION_BAR-CATEGORY_BAR-HEIGHT_TAB_BAR-12;
...
-(void)loadPickOrderList{
    ...
    [self.model loadPickOrderWithListType:self.list_type andModel:@"3"];
}
...
-(void)finishGoodsPick:(PickItemEntity *)entity{
    ...
    [self.tableView reloadData];
    [self.model finishPickPackage:type andOrderId:entity.order_id andDeliveryId:entity.sid andPackageType:package_type andModel:@"3"];
}
    ...
    if (buttonIndex == alertView.firstOtherButtonIndex) {
        ...
        if([txt_value isEqualToString:@"1234"]){
            ...
            if(all_delivery_ids==nil||all_delivery_ids.length<=0){
                ...
                [self startLoadingActivityIndicator];
                [self.model finishAllPackagePick:all_order_ids andDeliveryId:all_delivery_ids andModel:@"3"];
            }
